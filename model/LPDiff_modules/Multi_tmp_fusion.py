...
class MTA(nn.Module):
    ...
    def forward(self, lr_seq):
        ...
        if N == 1:
            ...
            # Hack pour connecter les poids au graphe (DDP find_unused_parameters=False)
            d = accumulated_feat.detach()
            dummy = (self.IntraframeAtt(d, d).sum() +
                     self.crossatt(d, d).sum())
                     
            # Multiplier par 0.0 connecte le graphe sans altérer la valeur réelle de l'output
            channel_output = channel_output + 0.0 * dummy
        else:
