...
class FeatureWiseAffine(nn.Module):
    ...
    def forward(self, x, noise_embed):
        ...
        if self.use_affine_level:
            ...
            x_noise_func = self.noise_func(noise_embed)
            total_dim = x_noise_func.shape[0] * x_noise_func.shape[1]
            dimen = total_dim // batch // channel // 2
...
if __name__ == '__main__':
    model = UNet(in_channel=6, out_channel=3, inner_channel=64, norm_groups=32, channel_mults=(1, 2, 4, 8, 8), attn_res=(16,), res_blocks=2, dropout=0.2, with_noise_level_emb=True, image_size=128)
    x = torch.randn(1, 6, 112, 224)
